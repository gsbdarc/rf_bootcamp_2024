...
1. **CPU (Central Processing Unit)**: The primary component that processes instructions and data. CPU does the work.
...
3. **Storage**: On the Yens, storage is a Network File System (NFS), called ZFS. Storage, also called disk(s) or hard drive(s), allows multiple computers (Yen nodes) to access files (i.e., CSV file) over a network as if they were on the local hard drive. Storage medium can be "spinning disks", HDDs which are slower to access because they have moving parts or faster flash-based disks or SSDs which are more expensive. For example, ZFS project space use HDD's but local to each node, we have flash based disks which can be accessed at `/tmp` path where a running program can output temporary files and read and write them very quickly.   
